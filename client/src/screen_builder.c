...
void build_all(GtkWidget **main_area)
{
    ...
    gtk_widget_set_name(GTK_WIDGET(t_screen.boxes[2]), "settingsbutton");
    gtk_widget_set_halign(GTK_WIDGET(t_screen.boxes[2]), GTK_ALIGN_CENTER);
    gtk_widget_set_size_request(GTK_WIDGET(t_screen.boxes[2]), 46, 46);
    gtk_box_pack_end(GTK_BOX(selection), t_screen.boxes[2], FALSE, FALSE, 10);
    tooltip("Settings", t_screen.boxes[2]);

    t_screen.boxes[3] = gtk_event_box_new();
    gtk_widget_set_name(GTK_WIDGET(t_screen.boxes[3]), "statusbutton");
    gtk_widget_set_halign(GTK_WIDGET(t_screen.boxes[3]), GTK_ALIGN_CENTER);
    gtk_widget_set_size_request(GTK_WIDGET(t_screen.boxes[3]), 42, 42);
    gtk_box_pack_end(GTK_BOX(selection), t_screen.boxes[3], FALSE, FALSE, 10);
    ...
    gtk_event_box_set_above_child(GTK_EVENT_BOX(t_screen.boxes[3]), TRUE);
    gtk_container_add(GTK_CONTAINER(t_screen.boxes[3]), avatar);
    ...
    g_signal_connect(G_OBJECT(t_screen.boxes[3]), "button_press_event", G_CALLBACK(status_click), NULL);
    ...
    g_signal_connect(G_OBJECT(t_screen.boxes[2]), "button_press_event", G_CALLBACK(settings_click), NULL);
}
